...
test -z "$ROOTFSTYPE" && ROOTFSTYPE=xfs
...
test -z "$BOOTFSTYPE" && BOOTFSTYPE=xfs
...
if test >>major<< -ge 7; then
    if test -d /sys/firmware/efi; then
        echo "bootloader --boot-drive=$DRIVE" >> /tmp/ks-part.cfg
        echo "part /boot/efi --ondisk $DRIVE --fstype=efi --size=$EFISIZE" >> /tmp/ks-part.cfg
    else
        echo "bootloader --location=mbr --boot-drive=$DRIVE" >> /tmp/ks-part.cfg
        echo "part biosboot --fstype=biosboot --size=1" >> /tmp/ks-part.cfg
    fi
else
    echo "bootloader --location=mbr" >> /tmp/ks-part.cfg
fi
